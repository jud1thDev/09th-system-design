...
- Kafka 토픽/파티션은 어떤 키로 나눌 것인가? 같은 videoId의 이벤트 순서는 어떻게 보장하는가?
...
- 키워드 매칭 점수(BM25)와 벡터 유사도 점수는 스케일이 전혀 다른데 어떻게 융합할 것인가?
...
- 조회수가 바뀔 때마다 문서를 재색인할 수는 없는데, 텍스트 색인과 동적 속성 저장소를 분리할 것인가?
...
- 동적 속성 갱신 이벤트가 초당 수십만 건인데, 어떻게 반영할 것인가?
...
- 조회수 거의 없는 롱테일 영상(전체의 대부분)을 어떻게 처리할 것인가?
