...
- 삭제는 "삭제됨" 표시 후 나중에 정리되는 구조라 그 전까지 색인에 남아 있는데, 1분 SLA를 어떻게 지킬 것인가?
...
- 질의 임베딩 생성(GPU 모델 호출)이 추가하는 지연은 얼마이고, 어떻게 줄일 것인가?
- 수십 개 샤드에 흩어 조회하고 모으는 구조에서는 가장 느린 샤드가 전체 지연을 결정하는데(tail latency), 어떻게 완화할 것인가?
- 인기 질의(컴백, 사건 직후 폭증)는 결과 캐시로 받을 수 있는가?
