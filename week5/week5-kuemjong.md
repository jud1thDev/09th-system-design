...
- 네트워크 단절 상황을 중앙 broker의 합의 구조에만 기대기보다 HA/replica와 Edge local buffer 중심으로 처리하는 것이 더 적합하지 않은가?
    ...
    - Kafka도 ISR, replication factor, `acks=all`, `min.insync.replicas`처럼 quorum에 가까운 복제 조건을 쓰지만, broker별 local disk replica를 두는 구조로 이해해야 하지 않는가?
        - Kafka의 ISR 기반 partition replica와 RabbitMQ quorum queue처럼 MQ별 복구/저장 모델이 다르므로, 단순히 quorum 여부만으로 안정성을 비교하면 안 되지 않는가?
        - remote shared volume에 의존하는 MQ 구조보다 broker별 local disk와 replica placement를 명확히 두는 쪽이 장애 전파를 줄이지 않는가?
...
- 중앙 메시징/저장 계층은 단일 디스크에 의존하지 않고 replica 기반으로 구성한다.
    - Kafka broker는 각 broker의 local disk에 partition replica를 두고, replication factor와 ISR로 내구성을 확보한다.
    - `acks=all`, `min.insync.replicas`를 사용하면 일정 수 이상의 replica에 기록된 경우에만 성공으로 보아 데이터 유실 가능성을 줄일 수 있다.
    - Kafka도 quorum에 가까운 복제 조건을 사용하지만, remote shared volume에 의존하기보다 broker별 local disk replica와 rack-aware placement를 우선한다.
    - 네트워크 partition으로 중앙 Kafka에 쓰기 어려운 구간은 Edge local buffer에 쌓고 복구 후 재전송하는 방식을 우선한다.
