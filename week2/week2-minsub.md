...
이번 `택시 호출 서비스 설계`에서 배운 관점은 '특정 서비스가 어떤 작업을 처리해야 하는가'에 적합한 서비스 및 데이터 구조를 설계하는 것이었습니다.

특히, 이런 실시간성 서비스가 포함된 경우 성능과 비용, 구조의 복잡도, 운영 복잡도를 모두 감안하고도 성능을 고려하는 것이 맞는가도 처음 고민해볼 수 있었고, RedisGEO나 RedisHash 등 처음 다뤄보는 생소한 DB도 공부해볼 수 있어서 좋았습니다.
아직 다 흡수하지 못한 체로 진행한 설계이고, 시스템을 바라보는 관점도 미흡하지만,,, 다음에는 더 공부해서 확신 있는 설계를 진행해보고 싶네요!
